...
app.post('/orders', auth, async (req, res) => {
  ...
  for (const i of items) if (Number(i.qty) > 0) await db.sql`INSERT INTO order_items(order_id,product_id,qty) VALUES (${order.rows[0].id},${Number(i.product_id)},${Number(i.qty)})`;
